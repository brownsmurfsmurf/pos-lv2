...
---|---|
| ![ログイン](sdr_fig08_1_ログイン.png) | ログイン: 画面 → BFF → API がパスワードを照合し JWT を発行 → BFF が Cookie に保存。失敗はどちらが違うか言わず 401 |
| ![スキャン追加](sdr_fig08_2_スキャン追加.png) | スキャン: バーコードを読む → API-03 で商品を探す → 同じ商品なら数量+1、なければ新しい行 → API-05 で再計算。未登録なら「商品がマスタ未登録です」 |
| ![会員値引き](sdr_fig08_3_会員値引き.png) | 会員読み込み: API-04 で会員を探す → 登録済みの行も含めて API-05 を呼び直し、値引きを付ける |
| ![購入確定](sdr_fig08_4_購入確定.png) | 購入確定: API-06 が取引日を 1 回取得して再計算・照合 → 一致なら 1 回のトランザクションで保存 → ポップアップ → 閉じて空にする |
